...
export default function NyuPage() {
    ...
    async function fetchQuestions() {
      ...
      if (data.length === 0) {
        const { error: insertError } = await supabase.from('questions').insert([
          {
            school: 'nyu',
            question_text: "What’s student life like at nyu?",
  ...
  return (
    <div style={{ padding: '2rem', fontFamily: 'Inter, sans-serif' }}>
      <h1 style={{ fontSize: '2rem', fontWeight: 'bold' }}>NYU Q&A Hub</h1>
      <p style={{ marginBottom: '1.5rem' }}>
        Real, anonymous questions answered by verified NYU students.
      </p>
      <div>
        {questions.map((q) => (
          <div
